...
总体的原则是：高内聚、低耦合。即模块内部的功能应该尽可能的紧密相关，不同模块之间的依赖关系应该尽可能的简单。
...
划分模块时，除了参考上游过程产生的流程图、需求设计文档以外，通常可以从以下角度来分析：
...
RPA软件中可以用于划分不同层级模块的功能如下，RPA工程师们可根据需要使用，越靠前的功能通常对应更顶层的模块划分。
