...
* 使程序具备良好的可维护性、可扩展性
  ...
  * 当某个模块内部尽快修改时，只要接口不变，那么调用它的模块就不会受到影响，不需要一起修改。
  * 某个模块不再需要时，可以直接去掉它。
* 方便进行单元测试
...
* 有利于团队间分工开发
  ...
  * 当一个大型项目被分割成较小的、模块化的部分时，它使团队成员之间的分工更容易。每个人或子团队可以专注于一个特定的模块，促进并行开发，加快整个项目的开发速度。
  * 这种划分也有助于问责，因为很清楚每个模块由谁负责。
...
* 数值为空，比如某些变量还没有被正确的赋值（Python中的None，UiBot / UiPath / Encoo / 数据库中的Null），就被使用了，就可能发生异常。
...
* 某个审查金融风险的程序，业务规定「遇到超过10000元，客户IP地址不在国内」的情况，就不应该让RPA程序自动处理，应该交给人工来判断。
